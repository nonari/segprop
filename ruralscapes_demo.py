...
def main(vid=None, start_from=None, stop_at=None):
    ...
    if iterations > 0:
        for video in videos:
            segprop.vote((os.path.join(PATH, 'flow_farneback', 'DJI_' + video + '_forward.h5'),
                         ...
                         precalc_flow=False, start_from=start_from, stop_at=stop_at+1,
                         device=device, overwrite=False, frame_filter=None)
    ...
    for it in range(2, iterations + 1):
        for video in videos:
            segprop.iterate((os.path.join(PATH, 'flow_farneback', 'DJI_' + video + '_forward.h5'),
                            ...
                            pv_series=[0, 5, 10], start_from=start_from, stop_at=stop_at+1, frame_copy=frame_copy,
                            device=device, overwrite=False, frame_filter=None)
    ...
    for video in videos:
        segprop.denoise((os.path.join(PATH, 'flow_farneback', 'DJI_' + video + '_forward.h5'),
                        ...
                        pv_series=[0, 1, 3, 5, 7], start_from=start_from, stop_at=stop_at+1, method='self',
                        frame_filter=None, device=device, overwrite=False)
